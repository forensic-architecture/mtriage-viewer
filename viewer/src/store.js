...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    [types.FETCH_BATCHES](state, allBatches) {
      state.allBatches = allBatches;
      state.fetching = false;
    },
  ...
  actions: {
    ...
    fetchFromBatch({ commit, state }, data) {
      ...
      api
        .fetchFromBatch(data.q, data.elements)
        .then((result) => {
    ...
    fetchAttribute({ commit }, data) {
      ...
      api
        .fetchAttribute(data.attribute, data.query)
        .then((result) => {
          commit(types.FETCH_ATTRIBUTE, {
